...
class Pulsynth:
    def __init__(self, octave, voices, spread, mul):
        ...
        self.src = [SineLoop(freq=100, feedback=0, mul=self.env[i], add=0) for i in range(self.voices)]
        self.voiceMix = Mix(input=self.src, voices=2, mul=1, add=0)

        self.delay = Delay(input=self.voiceMix, delay=[0.25, 0.5], feedback=0.25, maxdelay=0.5, mul=0.7, add=0)
        self.rvb = Freeverb(input=self.voiceMix+self.delay, size=0.90, damp=0.50, bal=0.5, mul=1, add=0)

        self.output = Mix(input=self.rvb, voices=2, mul=self.mul, add=0)

    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_2147 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_2147.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/////// Vectors:
                ...
                float4 node_2147 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_2147.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_2102.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal);
                o.posWorld = mul(_Object2World, v.vertex);
                float3 lightColor = _LightColor0.rgb;
                o.pos = mul(UNITY_MATRIX_MVP, v.vertex );
...
/////// Vectors:
                ...
                float3 _Normals_var = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(i.uv0, _Normals)));
                float4 node_2102 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_2102.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float3 normalLocal = normalize(lerp(_Normals_var.rgb,float3(0,0,1),node_133));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_3530.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal);
                o.posWorld = mul(_Object2World, v.vertex);
                o.pos = mul(UNITY_MATRIX_MVP, v.vertex );
...
/////// Vectors:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_9535 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_9535.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal);
                o.posWorld = mul(_Object2World, v.vertex);
...
/////// Vectors:
                ...
                float4 node_9535 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_9535.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
